...
export class MovieGenreDetailComponent {
  ...
  constructor(
    private _modelSrv: MovieGenreService,
  ...
  ngOnInit(): void {
    ...
    if (this.id) {
      this._modelSrv.getById(this.id).subscribe((movie_genre) => {
        this.form.patchValue(movie_genre);
  ...
  create() {
    this._modelSrv
      ...
      .subscribe(() => {
        ...
        this.router.navigate(['/', 'movie-genre']);
      });
  }
